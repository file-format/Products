...
<p>The open source library Miniaudio enables software developers to load and playback audio files inside their C applications. The library can be also used for recording purposes. When the apps send the request to the library to play the audio file, it uses the default playback device to playback the audio. The decoder is entirely decoupled from the device and can be used independently of it. The library also supports the playback of multiple files by simply loading multiple decoders and mixing them. </p>
